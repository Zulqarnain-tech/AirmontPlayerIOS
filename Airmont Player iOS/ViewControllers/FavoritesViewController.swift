...
extension FavoritesViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didHighlightRowAt indexPath: IndexPath) {
        ...
        if (indexPath.section == 0 && IPTV.hasChromecast()) {
            ...
            //let viewController = UIApplication.shared.windows.first!.rootViewController as! VideoPlayerViewController
            var source_id = 0
...
//            viewController.updateActivityIndicator()
            ...
            viewController.updateActivityIndicator()
            IPTV.setChannel(id: channels[indexPath.row].channel.id) {                    DispatchQueue.main.asyncAfter(deadline: .now() + 8) {
                    viewController.actionActivity = false
                    viewController.updateActivityIndicator()
                }
            }
